...
We consider an EMS suspension system, which consists of two parts: the rail and the magnet, which attract each other. The rail is a straight conductor e. g. of iron and the magnet is an actively controlled homogeneous electromagnet. The active control allows a (nearly) steady levitation under the rail with constant airgap, i. e. a balance between the electromagnetic force which pulls upwards and the gravitational force which pulls downwards.\
...
When the magnet is not moving, we expect a constant ($pdv(B, x)=0$) magnet field $B_0$ in z-direction in the aera between the rail and the magnetic yoke surface. Only at the front and at the end we have a change of magnetic flux along x-direction ($pdv(B, x) != 0$) to rise the magnetic from 0 up to $B_0$ at the front and to decline it at the end as depicted in @fig:static_B0.
#figure(caption: [Magnetic field profile along x-axis for a static magnet],
  // placement: left,
  image("../figures/models/static_B0.svg", width:70%)
)<fig:static_B0>
As the magnet is moving forward we expect a change in the magnetic flux profile, especially at the front and at the end of the magnet. The movement induces eddy currents in the rail, which encounter the magnet flux change by creating a magnetic field which opposes the change. This phenomenon can be described from the perspective of two diffrent frames: from the frame of a static rail and a moving magnet and from the frame of a moving rail and a static magnet.\
...
$y = plus.minus a$ which gives following solution
...
\
Regarding the exciting component of equation @eq:wave_equation_induced, neglecting $frac(partial^2 b_e, partial y^2)$ (since motion is only in $x$-direction) and considering $pdv(u(x),x)=delta(x)$ holds
#math.equation([$
