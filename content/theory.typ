...
=== Coulomb force
<coulomb-force>
Electrically charged particles like electrons and protons repel or attract
...
<magnetic-field-of-an-electric-current>
In this section, we will discuss the magnetic field generated by an electric current. We will start with Ampère's law, which describes the magnetic field generated by a current-carrying wire. Then we will derive the Biot-Savart law, which gives a more general expression for the magnetic field generated by an arbitrary current distribution.
...
$
  vb(B) \( vb(r) \) = frac(mu_0, 4 pi) integral curl frac(vb(J) \( vb(r)' \), \| vb(r) - vb(r)' \|) dd(r, 3)' med.
$<biot-savart-curl>
...
to:
$
  vb(B) \( vb(r) \) = frac(mu_0, 4 pi) integral frac(vb(J) \( vb(r)' \) times \( vb(r) - vb(r)' \), \| vb(r) - vb(r)' \|^3) dd(r, 3)' med.
$<biot-savart>
\
\
\
...
== Time variant fields
<time-variant-fields>
In this section, we will discuss time-variant fields and their transformation. We will start with Faraday's induction law, which describes how a changing magnetic field induces an electric field. Then we will discuss the transformation of fields, which is important when dealing with different reference frames.
=== Induction Law
<induction-law>
...
$
  integral.cont_C [vb(E)'-(vb(v)times vb(B))]dot dd(vb(l)) = - integral_S pdv(vb(B), t)dot dd(vb(n)) med.
$<induction_magnet_frame>
...
This form of equation is often found to describe diffusion processes, for example in the diffusion of heat in a solid or neutron diffusion.\
